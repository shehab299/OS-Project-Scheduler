...
void Scheduler::displayTerm()
{
	...
	while (!trmList.isEmpty())
	{
		cur = trmList.peek();
		cout << cur->getId() << ", ";
		temp.enqueue(trmList.peek());
		trmList.dequeue();
	}
	cout << endl;
	while (!temp.isEmpty())
	{
		trmList.enqueue(temp.peek());
		temp.dequeue();
	}
}

void Scheduler::displayRun()
{
	Processor* cur;
	for (int i = 0;i < processorList.getSize();i++)
	{
		cur = processorList.getElement(i);
		if (cur->isBusy())
		{
			cout << cur->getRunningProcess()->getId() << "(P" << i + 1 << "), ";
		}
	}
	cout << endl;
}
